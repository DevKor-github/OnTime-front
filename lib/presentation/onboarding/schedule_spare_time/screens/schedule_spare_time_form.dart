...
class _ScheduleSpareTimeFormState extends State<ScheduleSpareTimeForm> {
  ...
  void initState() {
    super.initState();
    context.read<ScheduleSpareTimeCubit>().initialize();
  }
  ...
  Widget build(BuildContext context) {
    ...
    return BlocBuilder<ScheduleSpareTimeCubit, ScheduleSpareTimeState>(
      builder: (context, state) {
        final cubit = context.read<ScheduleSpareTimeCubit>();
        return OnboardingPageViewLayout(
          title: AppLocalizations.of(context)!.setSpareTimeTitle,
          subTitle: RichText(
            text: TextSpan(
              text:
                  '${AppLocalizations.of(context)!.setSpareTimeDescription}\n',
          ...
          child: ScheduleSpareTimeField(
            lowerBound: cubit.lowerBound,
            spareTime: state.spareTime,
            minimumWarningMessage: AppLocalizations.of(
              context,
            )!.spareTimeMinimumWarning,
            onSpareTimeDecreased: cubit.spareTimeDecreased,
            onSpareTimeIncreased: cubit.spareTimeIncreased,
          ),
        );
      },
    );
