...
class StubGetNearestUpcomingScheduleUseCase
  ...
  StubGetNearestUpcomingScheduleUseCase(
    this.streamFactory, {
    this.includeInFutureQuery = true,
  });
  final bool includeInFutureQuery;
  ...
  Stream<NearestScheduleQuery> call({required NearestQueryKey key}) =>
      streamFactory().map((value) {
        _last = value;
        return nearestQueryFixture(includeInFutureQuery ? value : null, key);
      });
...
ScheduleWithPreparationEntity buildSchedule({
  ...
  return ScheduleWithPreparationEntity(
    ...
    preparationFrozen: startedAt != null,
...
// An active UI fixture contains both a durable start fact and its matching
// runtime projection. Merely receiving an upcoming appointment cannot start it.
Map<String, TimedPreparationSnapshotEntity> activeSnapshot(
  ScheduleWithPreparationEntity schedule,
  DateTime savedAt,
) {
  assert(
  ...
  return {
    schedule.id: TimedPreparationSnapshotEntity(
...
class StaticScheduleBloc implements ScheduleBloc {
  StaticScheduleBloc(this._state);
...
Future<void> pumpWithStaticScheduleBloc(
  WidgetTester tester, {
  required StaticScheduleBloc bloc,
  required GoRouter router,
}) {
  return pumpWithRouter(tester, bloc: bloc, router: router);
}
...
void main() {
  group('Preparation flow widgets', () {
    ...
    testWidgets(
      'official start prompt uses one action and opens alarm',
      (tester) async {
    ...
    testWidgets(
      'alarm prompt without cached schedule still offers early start',
      (tester) async {
        await setLargeTestViewport(tester);
        ...
        final router = GoRouter(
          ...
          routes: [
            GoRoute(
              path: '/scheduleStart',
              builder: (_, __) => const ScheduleStartScreen(
                promptVariant: ScheduleStartPromptVariant.alarm,
              ),
            ),
            GoRoute(
              path: '/alarmScreen',
        ...
        expect(
          find.textContaining('start preparing early now'),
          findsOneWidget,
        );
    ...
    testWidgets(
      'close button confirms before leaving start prompt',
      (tester) async {
        await setLargeTestViewport(tester);
        ...
        await pumpWithRouter(tester, bloc: bloc, router: router);
        await tapAndPump(tester, find.byIcon(Icons.close));
    ...
    testWidgets(
      ...
      (tester) async {
        ...
        try {
          ...
          await tester.runAsync(alarmBloc.close);
        }
      },
      timeout: const Timeout(Duration(seconds: 15)),
    );
    ...
    testWidgets(
      'upcoming alarm screen can start preparing immediately',
      (tester) async {
        await setLargeTestViewport(tester);
        final schedule = buildSchedule(
          id: 's-upcoming-alarm-start',
        ...
        final staticBloc = StaticScheduleBloc(ScheduleState.upcoming(schedule));
        ...
        await pumpWithStaticScheduleBloc(
          tester,
          bloc: staticBloc,
          router: router,
        );
        await pumpUntilFound(tester, find.text('Start Preparing'));

        expect(find.text('Meeting'), findsOneWidget);
        expect(
          find.textContaining('Preparation starts in 5 minutes'),
          findsOneWidget,
        );
        ...
        expect(
          staticBloc.addedEvents,
          contains(isA<SchedulePreparationStarted>()),
        );
      },
      timeout: const Timeout(Duration(seconds: 15)),
    );
    ...
    testWidgets(
      'upcoming alarm screen home button returns home without starting',
      (tester) async {
        await setLargeTestViewport(tester);
        final schedule = buildSchedule(
          id: 's-upcoming-alarm-home',
          scheduleTime: now.add(const Duration(minutes: 50)),
          steps: const [
            PreparationStepWithTimeEntity(
              id: 'p1',
        ...
        expect(
          staticBloc.addedEvents,
          isNot(contains(isA<SchedulePreparationStarted>())),
        );
      },
      timeout: const Timeout(Duration(seconds: 15)),
    );
    ...
    testWidgets(
      ...
      (tester) async {
        ...
        final schedule = buildSchedule(
          // This existing run began at its planned boundary 15 minutes ago.
        ...
        final alarmBloc = ScheduleBloc.test(
          ...
          createSessionUseCase(
            saveUseCase: NoopSaveTimedPreparationUseCase(),
            getSnapshotUseCase: StubGetTimedPreparationSnapshotUseCase(
              activeSnapshot(schedule, schedule.startedAt!),
            ),
            clearTimedUseCase: NoopClearTimedPreparationUseCase(),
            startUseCase: startUseCase,
            finishUseCase: finishUseCase,
    ...
    testWidgets(
      'completion dialog continue shows live leave countdown for restored durable flow',
      (tester) async {
        await setLargeTestViewport(tester);
