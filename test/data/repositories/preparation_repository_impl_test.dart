...
void main() {
  group('stream-backed behavior', () {
    ...
    test(
      ...
      () async {
        ...
        expect(remoteDataSource.updatedSpareTimes, [
  ...
  group('updateDefaultPreparation persistence checks', () {
    ...
    test('calls update and reloads persisted default preparation', () async {
      await preparationRepository.updateDefaultPreparation(tPreparationEntity);

      expect(remoteDataSource.updatedDefaultPreparations, [tPreparationEntity]);
      expect(remoteDataSource.getDefaultCallCount, 1);
    });

    test('throws when backend does not persist updated preparation', () async {
      ...
      remoteDataSource.persistUpdatedDefault = false;
      remoteDataSource.defaultPreparation = persistedPreparation;
    ...
    test('throws an exception if remote data source fails', () async {
      remoteDataSource.throwOnNext = true;
...
class _FakePreparationRemoteDataSource implements PreparationRemoteDataSource {
  ...
  int getDefaultCallCount = 0;
  bool persistUpdatedDefault = true;
  ...
  Future<void> updateDefaultPreparation(
    ...
    if (persistUpdatedDefault) {
      defaultPreparation = preparationEntity;
    }
  }
