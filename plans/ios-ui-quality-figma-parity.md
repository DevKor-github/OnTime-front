...
- Created a route and variant inventory with hash-verified Figma images. Six route defaults and 16 variants are visually reviewed; five app-only routes without a canonical shell are excluded from Figma visual parity, nine route defaults remain pending, and two Home variants are in progress with seven variants pending. The manifest and `dart run tool/report_figma_ui_coverage.dart` are the current coverage source.
...
- Added date injection for deterministic Home fixtures and month-only loading/error overlays. Loading blocks date taps; error retries the focused month while Home's hero and today card remain mounted. Full-shell visual comparison remains pending because the scheduled today card still differs from the Figma source.
- `flutter analyze --no-pub`, the local-only boundary and generated-Dart checks, and 587 Flutter tests with coverage (single-concurrency run) passed. Coverage is 84.92% against the CI minimum of 80%. The iOS simulator build was attempted twice and failed because the host disk ran out of space; no native launch or physical-device profile is claimed.
