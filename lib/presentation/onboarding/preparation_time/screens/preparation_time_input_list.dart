        ...
        child: BlocBuilder<PreparationTimeCubit, PreparationTimeState>(
          builder: (context, state) {
            return ListView.builder(
              itemCount: state.preparationTimeList.length,
              itemBuilder: (context, index) {
                final value = state.preparationTimeList[index];
                return Tile(
                  style: TileStyle(
                  ...
                  trailing: Row(
                    children: [
                      GestureDetector(
                        child: Text(
                            (value.preparationTime.inMinutes < 10 ? '0' : '') +
                                value.preparationTime.inMinutes.toString()),
                        onTap: () {
                          context.showCupertinoMinutePickerModal(
                            title: '시간을 선택해주세요',
                            initialValue: value.preparationTime,
                            onSaved: (value) {
                              context
