...
class _Input extends BackupRestoreInput {
  _Input({this.failCleanup = false});
  final bool failCleanup;
  @override
  Future<void> dispose() async {
    if (failCleanup) {
      throw RestoreStagingCleanupFailure(
        cleanupError: StateError('synthetic unlink failure'),
      );
    }
  }
...
class _Backup implements BackupOperationsPort {
  ...
  Future<BackupRestoreInput?> preview(String password) async =>
      _Input(failCleanup: failCleanup);
  @override
...
void main() {
  testWidgets('KO EN 200 percent data workflow states', (tester) async {
    final output = Directory(Platform.environment['A09_CAPTURE_DIR']!);
    ...
    for (final language in ['ko', 'en']) {
      ...
      expect(
        find.text(language == 'ko' ? '남은 처리 다시 시도' : 'Retry remaining work'),
        findsOneWidget,
      );
      ...
      await tester.pumpAndSettle();
      expect(port.applications, 1);
      expect(
        find.textContaining(
          language == 'ko' ? '임시 파일 정리' : 'temporary file cleanup',
        ),
        findsOneWidget,
      );
      expect(
      ...
      await capture('staging-cleanup-failed');
    }
  });
}
