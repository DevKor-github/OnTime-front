...
class PreparationNameCubit extends Cubit<PreparationNameState> {
  PreparationNameCubit({required this.onboardingCubit})
  ...
  void initialize() {
    List<PreparationStepNameState> preparationStepList = onboardingCubit
        .state
        .preparationStepList
        .map((e) {
  ...
  void preparationStepCreated(PreparationStepNameState stepState) {
    if (state.status == PreparationNameStatus.adding) {
      ...
      if (stepState.preparationName.isValid) {
        preparationStepList = [...state.preparationStepList, stepState];
      } else {
  ...
  void preparationStepNameChanged({required int index, required String value}) {
    final preparationStepList = List<PreparationStepNameState>.from(
  ...
  void preparationStepSelectionChanged({required int index}) {
    final preparationStepList = List<PreparationStepNameState>.from(
      state.preparationStepList,
    );
    final preparationStep = preparationStepList[index];
  ...
  bool _validate(List<PreparationStepNameState> preparationStepList) {
    final selectedPreparationStepList = preparationStepList
        .where((element) => element.isSelected)
        .toList();
    final isValid =
        selectedPreparationStepList.isNotEmpty &&
        Formz.validate(
